...
func TestWorkloadBytesSingleBasic(t *testing.T) {
	...
	assert.Equal(t, workloadBytes.Read.Counter, int64(1521265096))
	assert.Equal(t, workloadBytes.Read.Hz, 3361.9899999999998)
	assert.Equal(t, workloadBytes.Read.Roughness, 1333.3800000000001)
	assert.Equal(t, workloadBytes.Written.Counter, int64(15203569))
...
func TestWorkloadKeysSingleBasic(t *testing.T) {
	...
	assert.Equal(t, workloadKeys.Read.Counter, int64(4192507))
...
func TestWorkloadOperationsSingleBasic(t *testing.T) {
	...
	assert.Equal(t, workloadOperations.LocationRequests.Counter, int64(1564))
	...
	assert.Equal(t, workloadOperations.LowPriorityReads.Counter, int64(0))
	...
	assert.Equal(t, workloadOperations.MemoryErrors.Counter, int64(0))
	assert.Equal(t, workloadOperations.MemoryErrors.Hz, 0.0)
	assert.Equal(t, workloadOperations.MemoryErrors.Roughness, 0.0)
	assert.Equal(t, workloadOperations.ReadRequests.Counter, int64(8821994))
	...
	assert.Equal(t, workloadOperations.Reads.Counter, int64(8821994))
	assert.Equal(t, workloadOperations.Reads.Hz, 21.1999)
	assert.Equal(t, workloadOperations.Reads.Roughness, 7.0472700000000001)
	assert.Equal(t, workloadOperations.Writes.Counter, int64(114099))
...
func TestWorkloadTransactionsSingleBasic(t *testing.T) {
	...
	assert.Equal(t, workloadTransactions.Started.Counter, int64(2473210))
	...
	assert.Equal(t, workloadTransactions.StartedBatchPriority.Counter, int64(637))
	...
	assert.Equal(t, workloadTransactions.StartedDefaultPriority.Counter, int64(1909130))
	...
	assert.Equal(t, workloadTransactions.StartedImmediatePriority.Counter, int64(563443))
